...
EXP_NAME="${1:-pi05_libero_jax_8gpu_offline}"
LOG_FILE="$PROJECT_ROOT/train_${EXP_NAME}.log"

# Local dataset root (contains libero/).
...
# Quick checks.
[[ -f "$NORM_STATS_PATH" ]] || {
  echo "[ERROR] Missing local norm stats: $NORM_STATS_PATH"
  exit 1
}
[[ -f "$JAX_BASE_PARAMS/_METADATA" ]] || {
  echo "[ERROR] Missing local JAX base params: $JAX_BASE_PARAMS"
  exit 1
}
